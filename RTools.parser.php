<?php
	...
	$rtoolsMarkerList = array();
	
	$rtoolsScripts = array();
	$rtoolsGeoLocates = array();
	...
	function efRCodeMBInArray($val, $arr) {
		foreach ($arr as $v){
	...
	function efRCodeRender( $input, $args, $parser, $frame = null ) {
	        ...
	        $wgOut->addModules( 'ext.RTools.parser' );
	    	 ...
	    	 wfLoadExtensionMessages('RTools');
	        
	        $form_class = '';
	        
	        $escaped_input = htmlspecialchars( $input );

	        $markercount = count($rtoolsMarkerList);
	        $id = time() + $markercount;
	        ...
	        if (! empty($args) && isset($args['variables']))
	        {
	        	...
	        	$tmp = efRCodeParseVariables($form_name, $code_name, $vars, false, (! empty($args) && isset($args['live']) && intval($args['live']) == 1 ? $submit_cmd : false));
	        	$variables .= $tmp[0];
	        ...
	        {
	        	$vars = $parser->recursiveTagParse( $args['variables'], $frame );
	        	$tmp = efRCodeParseVariables($form_name, $code_name, $vars, true);
	...
	function efRCodeParseGeoLocates()
	{
		...
		foreach($rtoolsGeoLocates as $id)
	...
	function efRCodeParseVariables($form_name, $code_name, $vars, $hidden = false, $live = false)
	{
		...
		if (preg_match($pattern, $vars))
		{
        	...
        	while ($var_i < count($vars))
        	{
	        	...
	        	while (isset($vars[$var_i]) and ($aname = efRCodeAttributeName($vars[$var_i])) and ! in_array($aname, array_keys($attributes)))
	        	{
	        		$attributes[$aname] = efRCodeAttributeValue($vars[$var_i]);
	        		$var_i ++;
	        	}
				...
				#	$qres = mysql_query("select input_value from rtools_inputs where user_id = $user_id and page_id = $articleID and code_name = '$name' order by time desc limit 1;",$obcWikiCon);
					...
					if (mysql_num_rows($qres->result))
						$default =  mysql_result($qres->result, 0);
				}
	        	...
	        	if (! $hidden && isset($attributes['category']))
	        	{
	        		...
	        		if ($cat_conditions != '')
	        			$ret .= efRCodeParseCategoryConditions($cat_conditions, $form_name, $id);
	        	}
	...
	function efRCodeParseCategoryConditions($conds, $form_name, $id)
	{
		...
		$rtoolsScripts[] = 'if (jQuery.inArray($(document.forms["'.$form_name.'"].elements["variable['.$var_name.']"]).val(),["'.join('","',$conds).'"]) == -1) mw.RTools.hide_varbox("'.$id.'")';
		$rtoolsScripts[] = '$(document.forms["'.$form_name.'"].elements["variable['.$var_name.']"]).change(function(){if (jQuery.inArray($(this).val(),["'.join('","',$conds).'"]) >= 0) mw.RTools.show_varbox("'.$id.'"); else mw.RTools.hide_varbox("'.$id.'");})';
	...
	function efRCodeAttributeName($str)
	{
		$tmp = explode(':',$str);
		return trim($tmp[0]);
	}
	
	function efRCodeAttributeValue($str)
	{
		$tmp = explode(':',$str);
	...
	function efRCodeParserAfterTidy($parser, &$text) {
        // find markers in $text
        // replace markers with actual output
        global $rtoolsMarkerList;
        $keys = array();
        $marker_count = count($rtoolsMarkerList);
 
        for ($i = 0; $i < $marker_count; $i++) {
                $keys[] = 'xx-RTools-marker' . $i . '-xx';
        }
 
        $text = str_replace($keys, $rtoolsMarkerList, $text);
        return true;
	}
